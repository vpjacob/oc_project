...
KNBBillingRecordSliderViewDelegate
>
...
@property (nonatomic, strong) UIView *abroad;
@property (nonatomic, strong) KNBBillingRecordSliderView *sliderView;
...
@implementation JFCityViewControllers
...
#pragma mark - KNBBillingRecordSliderViewDelegate
...
- (NSMutableArray *)characterMutableArray {
    if (!_characterMutableArray) {
        _characterMutableArray = [NSMutableArray arrayWithObjects:@"定位", @"历史", @"热门", nil];
    }
...
- (void)cityNameWithSelected:(BOOL)selected {
    ...
    if (selected) {
        ...
        [_characterMutableArray insertObject:@"区县" atIndex:0];
...
- (UIView *)abroad{
    if (!_abroad) {
        _abroad = [[UIView alloc] init];
        _abroad.backgroundColor = [UIColor grayColor];
    }
    return _abroad;
}

- (KNBBillingRecordSliderView *)sliderView{
    if (!_sliderView) {
        CGFloat height = 50;
        _sliderView = [[KNBBillingRecordSliderView alloc] initWithFrame:CGRectMake(0, 64, kDeviceWidth,height) buttonTitles:@[@"国内",@"海外"]];
        _sliderView.delegate = self;
        CALayer *TopBorder = [CALayer layer];
        TopBorder.frame = CGRectMake(0.0f, height - 0.5, kDeviceWidth, 0.5f);
        TopBorder.backgroundColor = [UIColor blackColor].CGColor;
        [_sliderView.layer addSublayer:TopBorder];
    }
    return _sliderView;
}
...
//        CGFloat scrollerViewHeight = kDeviceHeight - 64 - 50;
        _rootTableView = [[UITableView alloc] initWithFrame:self.scrollView.frame style:UITableViewStylePlain];
...
- (UIScrollView *)scrollView{
    if (!_scrollView) {
        CGFloat scrollerViewHeight = kDeviceHeight - 64 - 50;
        _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 64 + 50, kDeviceWidth, scrollerViewHeight)];
        ...
        self.rootTableView.frame = CGRectMake(0, 0, kDeviceWidth, scrollerViewHeight);
        self.selectVC = [[JJSelectAddressViewController alloc] init];
        self.selectVC.view.frame = CGRectMake(kDeviceWidth, 0, kDeviceWidth, scrollerViewHeight);
//        self.abroad.frame = CGRectMake(kDeviceWidth, 0, kDeviceWidth, scrollerViewHeight);
