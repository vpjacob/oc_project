...
//  queryDefaultAddress(urId);
	...
	function queryEggInfo(urId) {
		AjaxUtil.exeScript({
			...
			success : function(data) {
				...
				if (data.formDataset.checked == 'true') {
					$("#residueMoney").html($api.strToJson(data.formDataset.account).integral_balance);
				} else {
					api.toast({
						msg : data.formDataset.errorMsg
					});
	...
	function queryShoppingDeatilAndModelType() {
		...
		AjaxUtil.exeScript({
			...
			success : function(data) {
				console.log("积分商品支付详情" + $api.jsonToStr(data));
				if (data.formDataset.checked == 'true') {
...
//	queryProductDeatilAndModelType();
	...
	$('#apply').click(function() {
		...
		countAll = ($("#countAll").html()).split("积分")[0];
		var price = ($("#price").html()).split("积分")[0];
		var residueMoney=($("#residueMoney").html()).split("积分")[0];
		if (Number(countAll) > Number(residueMoney)) {
			api.toast({
				msg : "您的商品总价值大于您的剩余积分余额，请重新选择！"
			});
			return false;
		};
		//金蛋支付
...
//							merchantName : "北京小客网络科技有限公司",
						...
						success : function(formset) {
							if (formset.execStatus == "true") {
								...
								$.ajax({
									type : 'POST',
									url : rootUrls + '/xk/buyShopPayIntegral.do',
