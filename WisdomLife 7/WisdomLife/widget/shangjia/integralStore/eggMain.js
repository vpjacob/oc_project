...
apiready = function() {
	...
	//查找所有金蛋商品
	function queryIntegralShoppingList(pages,name,sales) {
		api.showProgress({});
		AjaxUtil.exeScript({
			script : "mobile.shopping.shopping",
			needTrascation : false,
			funName : "queryIntegralShoppingList",
			form : {
				name : name,
				sales : sales,
				p : pages
					...
					if (list.length == undefined || list.length == 0 || list == undefined || list == '' || list.length == '') {
						...
						for (var i = 0; i < list.length; i++) {
							nowli +='<div class="same">'
								...
								+'<div class="busprice">'
								+'<span class="symbol">'+list[i].price_discount+'积分</span>'
								+'</div>'
	...
	queryIntegralShoppingList(page,"","");
//全部商品的下拉加载
...
//	alert(searchVal);
		queryIntegralShoppingList(page,searchVal,"");
		api.addEventListener({
			name : 'scrolltobottom'
		}, function(ret, err) {
			if (parseInt(page) <= parseInt(pageCount)) {
		...
		$(divs[i]).click(function() {
			...
			if ($(this).attr("class") == "special") {
				...
				if ($(this).attr("data") == 0) {
					...
					queryIntegralShoppingList(page,"",saleData);
					api.addEventListener({
						name : 'scrolltobottom'
					}, function(ret, err) {
						if (parseInt(page) <= parseInt(pageCount)) {
