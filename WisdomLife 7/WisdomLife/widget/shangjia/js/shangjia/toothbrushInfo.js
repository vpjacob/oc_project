...
apiready = function() {
	...
	$('#numAdd').click(function() {
		...
		if (parseInt(amout) < 999) {
			...
			$("#amout").html(amout);
			$('#countAll').html(parseInt(amout) * 698 + '元');
		}
	});
...
function changeColor(){
		if($('#colorChoose option:selected').val()==0 || $('#colorChoose option:selected').val()==1){
			$('#businessImg').attr('src','../image/address/toothbrush1.png');
			$('#content').html('小客k100声波电动牙刷  玫瑰金');
		}else{
			$('#businessImg').attr('src','../image/address/toothbrush2.png');
			$('#content').html('小客k100声波电动牙刷  拉丝银');
		}
	}
