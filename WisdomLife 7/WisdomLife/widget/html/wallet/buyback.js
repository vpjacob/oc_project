...
apiready = function() {
	...
	function getDealNo() {
		...
		var currentdate = date.getFullYear() + month + strDate + strHours + strMinutes + strSeconds + strMilliseconds;
		...
		AjaxUtil.exeScript({
			...
			success : function(data) {
				...
				if (data.formDataset.checked == 'true') {
					...
					var nowli = '<div class="jia" id="' + list.id + '">' + '<div class="left">' + '' + imgUlr + '' + '</div>' + '<div class="middle">' + '<div><span id="bankName">' + list.open_bank + '</span></div>' + '<div><span id="card_no">' + list.card_no + '</span></div>' + '</div>' + '</div>';
					$('#addcard').append(nowli);
